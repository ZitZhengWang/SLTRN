...
from model.relation_module import RelationModuleBasedOnTFOnBatch, RelationNetwork
...
def main():
    ...
    for task_num, (images, labels) in enumerate(train_loader):
        ...
        support_features_ext = support_features.unsqueeze(0).repeat(query_features.shape[0], 1, 1)  # [75,5,64]
        query_features_ext = query_features.unsqueeze(1)  # [75,1,64]
        # 数据样本方向拼接
        relation_pairs = torch.cat((query_features_ext, support_features_ext), dim=1)  # [75,6,64]

        # use transformer
        # 计算关系分数；
        relations = relation_network(relation_pairs, relation_pairs, args.tgt_mask, args.mask_type)  # 输入[75,6,64]    输入出【75,6】
        ...
        left_ones = Variable(torch.ones(query_features.shape[0]), 1).unsqueeze(1).to(device)  # [75,1]
        label = torch.cat((left_ones, one_hot_labels), dim=1)

        mse = nn.MSELoss().to(device)

        loss = mse(relations, label)
        ...
        if (task_num + 1) % 5000 == 0 or task_num == 0:
            ...
            with torch.no_grad():
                ...
                for i, (images, labels) in enumerate(val_loader):
                    ...
                    val_loss = mse(relations, label)

                    total_val_loss.append(val_loss.item())

                    _, predict_labels = torch.max(relations.data[:, 1:], 1)
