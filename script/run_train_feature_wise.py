...
# 202303012 18.31
# 测试特征图拼接输入的方式，+对角线的tgtmask
# 检查：首先，特征提取部分不适用全局池化层
#
# ========================================导入相关库===========================================
# 导入官方库
import os
...
from model.relation_module import RelationNetwork, RelationModuleBasedOnTFOnFeature
...
def main():
    ...
    model_dict = dict(
        CNNEncoder=backbone.CNNEncoder(use_GAP=False),
    ...
    # 构建关系模块并初始化
    if args.RelationModule == "RelationModuleBasedOnTFOnFeature":
        relation_network = RelationModuleBasedOnTFOnFeature(input_dim=args.transformerlayer_input_dim,
    ...
    for task_num, (images, labels) in enumerate(train_loader):
        ...
        # 前向计算 # images [5,17,3,224,224],输入shape为【way，shot，channal，dim】
        support_features, query_features = feature_encoder.parse_feature_map(images, CLASS_NUM, SHOT_NUM_PER_CLASS, QUERY_NUM_PER_CLASS)

        # 计算原型
        if args.aggregation == "proto_sum":
            support_features = torch.sum(support_features, 1).squeeze(1)
        elif args.aggregation == "proto_mean":
            support_features = torch.mean(support_features, 1).squeeze(1)   # 【5，64，361】

        # 【5*16，64，361]
        query_features = query_features.reshape(CLASS_NUM*VALIDATION_QUERY_NUM_PER_CLASS, *query_features.shape[2:])

        support_features_ext = support_features.unsqueeze(0).repeat(query_features.shape[0], 1, 1, 1)  # [75,5,64,361]
        query_features_ext = query_features.unsqueeze(1).repeat(1, support_features.shape[0], 1, 1)  # [75,5,64,361]
        # [400,128,361]
        relation_pairs = torch.cat((support_features_ext, query_features_ext), 2).reshape(-1, 2*query_features.shape[1],query_features.shape[2])  # [75*5,128,361]

        # use transformer
        # 计算关系分数；
        relations = relation_network(relation_pairs, relation_pairs, args.tgt_mask, args.mask_type)  # 输入[75,5,128]    输入出【75,5】
        ...
        if (task_num + 1) % 5000 == 0 or task_num == 0:
            ...
            with torch.no_grad():
                ...
                for i, (images, labels) in enumerate(val_loader):
                    ...
                    counter = 0
                    # 前向计算
                    support_features, query_features = feature_encoder.parse_feature_map(images, CLASS_NUM, SHOT_NUM_PER_CLASS, VALIDATION_QUERY_NUM_PER_CLASS)
                    # 计算原型
                    if args.aggregation == "proto_sum":
                        support_features = torch.sum(support_features, 1).squeeze(1)
                    elif args.aggregation == "proto_mean":
                    ...
                    # 【5*16，64，361]
                    query_features = query_features.reshape(CLASS_NUM * VALIDATION_QUERY_NUM_PER_CLASS,
                                                            *query_features.shape[2:])

                    support_features_ext = support_features.unsqueeze(0).repeat(query_features.shape[0], 1, 1,
                                                                                1)  # [75,5,64,361]
                    query_features_ext = query_features.unsqueeze(1).repeat(1, support_features.shape[0], 1,
                                                                            1)  # [75,5,64,361]
                    # [75*5,128,361]
                    relation_pairs = torch.cat((support_features_ext, query_features_ext), 2).reshape(-1, 2 * query_features.shape[1], query_features.shape[2])
