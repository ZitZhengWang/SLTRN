...
from model.relation_module import RelationModuleBasedOnTFOnBatch, RelationNetwork, RelationModuleBasedOnTEOnBatch
...
def main():
    ...
    model_dict = dict(
        ...
        Conv6WithGAP=backbone.Conv6WithGAP(),
    ...
    if args.use_trained_model:
        path_result = Path(args.checkpoint_dir)
    else:
    ...
    # 构建关系模块并初始化
    if args.RelationModule == "RelationModuleBasedOnTFOnBatch":
        relation_network = RelationModuleBasedOnTFOnBatch(input_dim=args.transformerlayer_input_dim,
                                                          head_num=args.head_num,
                                                          feedforeard_dim=args.transformerlayer_feedforward_dim,
                                                          norm_method=args.norm_method,
                                                          transformerlayer_num=args.transformerlayer_num,
                                                          hidden_size=args.hidden_size)
    elif args.RelationModule == "RelationModuleBasedOnTEOnBatch":
    ...
    if args.use_trained_model:
        ...
        feature_encoder_dir = path_checkpoint / f"feature_encoder_{args.checkpont_epoches // 5000.0}.pkl"
        relation_network_dir = path_checkpoint / f"relation_network_{args.checkpont_epoches // 5000.0}.pkl"

        feature_encoder.load_state_dict(torch.load(feature_encoder_dir))
        logfile.write(f"\n\tload feature encoder {args.checkpont_epoches // 5000.0} success")
        relation_network.load_state_dict(torch.load(relation_network_dir))
        logfile.write(f"\n\tload relation network {args.checkpont_epoches // 5000.0} success")
        total_val_loss = [0.1282]
        valid_accuracy = 0.5304
    ...
    for task_num, (images, labels) in enumerate(train_loader):
        if args.use_trained_model:
            task_num = task_num + args.checkpont_epoches
        ...
        # 计算关系分数；
        relations = relation_network(relation_pairs, relation_pairs, args.tgt_mask, args.mask_type)  # 输入[75,5,128]    输出【75,5】
        ...
        if (task_num + 1) % 5000 == 0 or task_num == 0:
            ...
            with torch.no_grad():
                ...
                for i, (images, labels) in enumerate(val_loader):
                    ...
                    relations = relation_network(relation_pairs, relation_pairs, args.tgt_mask,
                                                 args.mask_type)  # 输入[75,5,128]    输入出【75,5】
