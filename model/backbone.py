...
class CNNEncoder(nn.Module):
    ...
    该类为关系网络中原始的特征提取器；
    注意：由于初始化参数中的设置，类名中不要包含Conv，否则会报错
    """
    def __init__(self, use_GAP=False, input_channal=3):
        super(CNNEncoder, self).__init__()
        self.layer1 = nn.Sequential(
                        nn.Conv2d(input_channal,64,kernel_size=3,padding=0),
    ...
    def forward(self, x):
        ...
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)  # 输出为 【B,C,H,W】
        # out = out.view(out.size(0),-1)
        if self.use_GAP:
            out = self.GAP(out).squeeze()   # 输出为【B,C】
        return out    # [5,64]
...
class ConvNetSNopoolWithGAP(nn.Module): #Relation net use a 4 layer conv with pooling in only first two layers, else no pooling. For omniglot, only 1 input channel, output dim is [64,5,5]
    def __init__(self, depth):
        super(ConvNetSNopoolWithGAP,self).__init__()
        trunk = []
        for i in range(depth):
            indim = 1 if i == 0 else 64
            outdim = 64
...
def Conv6WithGAP():
    return ConvNetSNopoolWithGAP(6)
