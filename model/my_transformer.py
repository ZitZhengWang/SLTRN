...
from torch.nn import Module, Linear, BatchNorm1d, Dropout, MultiheadAttention, ModuleList, LayerNorm
...
class Transformer(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6, norm_method:str = "LayerNorm",
        ...
        if custom_encoder is not None:
            ...
            encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward, norm_method, dropout,
        ...
        if custom_decoder is not None:
            ...
            decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward, norm_method, dropout,
                                                    activation, layer_norm_eps, batch_first, norm_first,
                                                    **factory_kwargs)
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, norm_method:str = "LayerNorm", dropout: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.relu,
                 layer_norm_eps: float = 1e-5, batch_first: bool = False, norm_first: bool = False,
                 device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        ...
        elif norm_method == "BatchNorm1d":
            self.norm1 = BatchNorm1d(d_model)
            self.norm2 = BatchNorm1d(d_model)
        else:
            print("MyError:Unknown norm method!")
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        if self.norm_first:
            if self.norm_method == "LayerNorm":
                ...
                x = x + self._sa_block(self._add_norm1(x), src_mask, src_key_padding_mask)
                x = x + self._ff_block(self._add_norm2(x))
        else:
            if self.norm_method == "LayerNorm":
                x = self.norm1(x + self._sa_block(x, src_mask, src_key_padding_mask))
                x = self.norm2(x + self._ff_block(x))
            else:
                x = self._add_norm1(x + self._sa_block(x, src_mask, src_key_padding_mask))
                x = self._add_norm2(x + self._ff_block(x))
    ...
    def _add_norm1(self, x: Tensor) -> Tensor:
        # 输入【75，5，128】，交换后两个维度，做Batchnorm1d，完成之后再变回来
        x = self.norm1(x.transpose(1,2))
        return x.transpose(1,2)

    def _add_norm2(self, x: Tensor) -> Tensor:
        # 输入【75，5，128】，交换后两个维度，做Batchnorm1d，完成之后再变回来
        x = self.norm1(x.transpose(1,2))
        return x.transpose(1,2)
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
        ...
        if self.norm_first:
            if self.norm_method == "LayerNorm":
                ...
                x = x + self._sa_block(self._add_norm1(x), tgt_mask, tgt_key_padding_mask)
                x = x + self._mha_block(self._add_norm2(x), memory, memory_mask, memory_key_padding_mask)
                x = x + self._ff_block(self._add_norm3(x))
        else:
            if self.norm_method == "LayerNorm":
                ...
                x = self._add_norm1(x + self._sa_block(x, tgt_mask, tgt_key_padding_mask))
                x = self._add_norm2(x + self._mha_block(x, memory, memory_mask, memory_key_padding_mask))
                x = self._add_norm3(x + self._ff_block(x))
        return x
    ...
    def _add_norm3(self, x: Tensor) -> Tensor:
        # 输入【75，5，128】，交换后两个维度，做Batchnorm1d，完成之后再变回来
        x = self.norm1(x.transpose(1,2))
        return x.transpose(1,2)